...
client.on("ready", () => {
  ...
  client.user.setActivity(`forli`, { type: "WATCHING" });
});
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@434328002604564495>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "ولكم ") {
    message.channel.send("**Welcome to Forli,,,Enjooy**");
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    const wlcImage = await fsn.readFile(""); //اسم الصورة
